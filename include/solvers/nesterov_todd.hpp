...
private:
    ...
    optMatrix<prec_type> hvp(const Eigen::Ref<const optMatrix<prec_type>>& vecs, ProblemData<prec_type>& problem_data){
        optVector<prec_type> scaling_point = problem_data.cones.get_scaling_point();
...
public:
    ...
    ESDEState<prec_type> step(const ESDEState<prec_type>& esde_state, ProblemData<prec_type>& problem_data){
        ...
        optVector<prec_type> scaled_variable = problem_data.cones.get_scaled_variable();
        ...
        prec_type alpha_p = problem_data.cones.get_nt_step_length(pred_dir.s, pred_dir.z);
