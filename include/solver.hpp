...
private:
    ...
    void print_row(Model<prec_type>& model, int, const std::chrono::duration<double>&, const std::chrono::duration<double>&, const std::chrono::duration<double>&) const;
...
public:
    Point<prec_type> solve(Model<prec_type>& model, const prec_type& tol_gap,
        const prec_type& tol_fail, const int max_steps = 1000, const int record_every = 100);
};
